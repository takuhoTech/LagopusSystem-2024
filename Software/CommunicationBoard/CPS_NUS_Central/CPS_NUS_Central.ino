...
BLEClientService        CPS(UUID16_SVC_CYCLING_POWER); //0x1818
BLEClientCharacteristic CPM(UUID16_CHR_CYCLING_POWER_MEASUREMENT); //0x2A63
...
void setup() {
  ...
  CPS.begin();
  CPM.setNotifyCallback(notify_callback);
  CPM.begin();
...
void connect_callback(uint16_t conn_handle)
{
  ...
  //Bluefruit.Scanner.start(0);
  //これがあるとCyclingPowerのnotifyを受信しながらscanの継続が可能,Bluefruit.begin(0, 1);の引数には注意
